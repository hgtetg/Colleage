...
// D1 exec() treats newline-delimited SQL as separate statements. Keep each
// migration statement complete and execute it through prepare().run() instead.
const SCHEMA_STATEMENTS = [
  ...
  `INSERT OR IGNORE INTO subjects (id, course_id, name, code, icon, description) VALUES
...
export async function onRequestGet({ env, request }) {
  if (!env.DB) return htmlPage({ bound: false, ready: false, missing: REQUIRED_TABLES }, 503);
  try {
    return htmlPage({ bound: true, ...(await schemaStatus(env.DB)) }, 200);
  } catch (error) {
    return htmlPage({ bound: true, ready: false, missing: REQUIRED_TABLES, error: messageOf(error) }, 500);
  }
}
...
export async function onRequestPost({ env, request }) {
  ...
  try {
    ...
    if (!before.ready) await initializeSchema(env.DB);
    ...
    return json({ ok: false, error: 'Database initialization failed.', detail: messageOf(error).slice(0, 700) }, 500);
  }
}
...
async function initializeSchema(db) {
  for (let i = 0; i < SCHEMA_STATEMENTS.length; i += 1) {
    try {
      await db.prepare(SCHEMA_STATEMENTS[i]).run();
    } catch (error) {
      throw new Error(`Schema statement ${i + 1} failed: ${messageOf(error)}`);
    }
  }
}
...
function htmlPage(status, statusCode) {
  ...
  const missing = status.missing?.length ? `<div class="box"><strong>Missing tables</strong><p>${escapeHtml(status.missing.join(', '))}</p></div>` : '';
