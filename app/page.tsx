...
export default function HomePage() {
  ...
  const updateCodeStatus = async (status: 'active' | 'paused' | 'invalid') => {
    try {
      await mutate('toggle_code', { status });
      setNotice(
        status === 'paused'
          ? 'Student joins are paused.'
          : 'Student joins are active.',
      );
    } catch {}
  };
      ...
      <main className="page" id="top">
        ...
        {active === 'subjects' && (
          <Subjects
            ...
            canManage={campus?.viewer?.role === 'representative'}
          />
        )}
        ...
        {active === 'manage' &&
          ...
          campus?.viewer?.role === 'admin' ? (
            <Manage
              code={studentCode}
              status={codeStatus}
              setStatus={updateCodeStatus}
              changeCode={changeCode}
              addContent={addSubject}
              people={livePeople}
              saving={saving}
            />
          ) : (
          ...
          <aside className="drawer" aria-label="Campus Hub menu">
            ...
            >
              ...
              <div>
                ...
                <small>
                  {campus?.viewer
                    ? `${campus.viewer.role === 'representative' ? 'Representative' : 'Student'} · verified`
            ...
            <nav>
              ...
              >
                ...
                <span>
                  ...
                  <small>Members, codes & analytics</small>
                </span>
              </button>
        ...
        action={
          <button className="primary-button" type="button" onClick={onManage}>
            {viewer?.role === 'representative' ? (
              <>
                <BarChart3 size={17} /> Manage course
              </>
            ) : (
              <>
